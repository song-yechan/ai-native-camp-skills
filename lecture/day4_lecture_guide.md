...
───────────────────────────────────────────────
여러분이 3일간 만든 것들은 독립된 기능이 아니라,
AI를 원하는 방향으로 이끄는 하나의 시스템입니다.
```

> **발표자 노트**: 이 다이어그램이 3일간의 학습을 하나로 묶는 핵심입니다. CLAUDE.md(Day 1) + MCP(Day 2) + Skill(Day 3)이 합쳐져서 Harness라는 시스템이 되고, Compound(Day 4)는 그 시스템을 계속 개선하는 과정임을 강조하세요.
...
### 섹션 5: 스킬 공유와 서비스화 (15분)

#### 슬라이드 5-1: 섹션 타이틀
```
스킬 공유와 서비스화

"나만 쓰는 것 → 팀이 쓰는 것 → 세상이 쓰는 것"
```

#### 슬라이드 5-1b: 스킬 공유의 기본 — Git
```
스킬 공유의 기본: Git
...
2. 동료가 git clone 또는 ln -sf로 가져다 쓰기
3. 끝!

───────────────────────────────────────────────
이것만으로도 팀 내 스킬 공유가 가능합니다.

💡 "내 스킬 GitHub에 올려줘" 한마디면 됩니다.
```

> **발표자 노트**: 이 부분이 가장 현실적인 시작점입니다. Vercel/Supabase는 보너스이고, Git push → clone만으로도 충분히 팀에 기여할 수 있다는 점을 강조하세요.
...
───────────────────────────────────────────────
"시간이 되면 스킬에 살을 붙여 서비스로 만들 수도 있다"
```

#### 슬라이드 5-3: 3가지 인프라의 역할 (보너스)
```
3가지 인프라의 역할
...
#### 슬라이드 6-2: 실습 A — 1회전: CLAUDE.md 개선 사이클 (필수)
```
실습 A: 1회전 — CLAUDE.md 개선 사이클 (10분)
...
#### 슬라이드 6-3: 실습 B — 2회전: 스킬 개선 사이클 (필수)
```
실습 B: 2회전 — 스킬 개선 사이클 (필수, 10분)
...
───────────────────────────────────────────────
💡 Day 3 스킬이 없다면?
   → CLAUDE.md에 새 규칙을 하나 더 추가해보세요
   2회전을 돌리는 것 자체가 핵심입니다!
```
...
┌─────────────────────────────────────────────────────┐
│  ☐ 1. CLAUDE.md 규칙 확인                          │
│     필요한 규칙이 빠져있거나 충돌하는 건 없는지?    │
├─────────────────────────────────────────────────────┤
│  ☐ 2. SKILL.md 지시 확인                           │
│     "적절히", "알아서" 같은 모호한 표현은 없는지?   │
├─────────────────────────────────────────────────────┤
│  ☐ 3. MCP 연결 상태 확인 (/mcp)                    │
│     도구 연결이 정상인지?                           │
├─────────────────────────────────────────────────────┤
...
💡 대부분의 문제는 1~2번에서 해결됩니다.
   "AI가 못하는 게 아니라 내가 잘 안 알려준 것"
```

> **발표자 노트**: 이 체크리스트는 Compound 사이클의 Debug 단계에서 활용할 수 있는 실용적 도구입니다. 실습 중 문제가 발생하면 이 순서대로 확인하도록 안내하세요.
...
Compound 사이클 실습:
☐ 1회전: CLAUDE.md에 규칙 추가 완료 (필수)
☐ 새 대화에서 규칙 적용 확인
☐ 2회전: Day 3 스킬 출력 형식 수정 (필수)
...
| 0:10 | 0:20 | 섹션 1 | Day 1-3 리캡 | 4일 여정 빠르게 훑기 |
| 0:20 | 0:45 | 섹션 2 | Context Engineering | 개념 중심 |
